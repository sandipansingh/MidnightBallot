/**
 * Deploy MidnightBallot to a Midnight network (use --network preview|preprod).
 *
 * Non-interactive: npm run deploy:preview runs straight through.
...
// Identifier under which this contract's private witness state is stored.
const PRIVATE_STATE_ID = 'midnightBallotPrivateState';
...
const compiledContract = CompiledContract.make('counter', ContractModule.Contract).pipe(
  CompiledContract.withWitnesses({
    privateBallot: (context: any) => [context.privateState, context.privateState] as [any, any],
  } as never), // Dynamic import erases the generated Contract witness type.
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'midnight-ballot-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy MidnightBallot to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  console.log('  ℹ  Reporting each wallet\'s applied and target index every 15 seconds.\n');
  const syncStart = Date.now();
  let latestState: any;
  let rejectSync!: (error: Error) => void;
  const syncFailure = new Promise<never>((_, reject) => { rejectSync = reject; });
  const stateSubscription = walletCtx.wallet.state().subscribe({
    next: (value) => { latestState = value; },
    error: (error) => { rejectSync(error instanceof Error ? error : new Error(String(error))); },
  });
  let previousProgress = '';
  let lastProgressAt = Date.now();
  const syncStallMs = Number(process.env.MIDNIGHT_SYNC_STALL_MS) || 180_000;
  const syncMaxMs = Number(process.env.MIDNIGHT_SYNC_MAX_MS) || 0;
  const syncInterval = setInterval(() => {
    const elapsed = Math.round((Date.now() - syncStart) / 1000);
    const positions = latestState ? [
      ['shielded', latestState.shielded.progress.appliedIndex, latestState.shielded.progress.highestRelevantWalletIndex],
      ['unshielded', latestState.unshielded.progress.appliedId, latestState.unshielded.progress.highestTransactionId],
      ['dust', latestState.dust.progress.appliedIndex, latestState.dust.progress.highestRelevantWalletIndex],
    ] as const : [];
    const progress = positions.map(([name, applied, highest]) =>
      `${name} ${String(applied)}/${String(highest)}`,
    ).join(' | ');
    console.log(`  ⏳ Sync ${elapsed}s: ${progress || 'waiting for first state'}`);
    if (progress && progress !== previousProgress) {
      previousProgress = progress;
      lastProgressAt = Date.now();
    } else if (Date.now() - lastProgressAt > syncStallMs) {
      rejectSync(new Error(`Wallet sync made no progress for ${Math.round(syncStallMs / 1000)}s. Last state: ${progress || 'none'}`));
    }
    if (syncMaxMs > 0 && Date.now() - syncStart > syncMaxMs) {
      rejectSync(new Error(`Wallet sync reached its ${Math.round(syncMaxMs / 1000)}s limit. Last state: ${progress || 'none'}`));
    }
  }, 15_000);
  let state: Awaited<ReturnType<typeof walletCtx.wallet.waitForSyncedState>>;
  try {
    state = await Promise.race([walletCtx.wallet.waitForSyncedState(), syncFailure]);
  } catch (error) {
    ...
    if (initialTNight === 0n) {
      ...
      while (true) {
        ...
        if (Date.now() - start > timeoutMs) {
          ...
          console.log(`  Faucet:  ${networkConfig.faucet}`);
          console.log('  Re-run npm run deploy:preview after funding — your seed is preserved.\n');
          await walletCtx.wallet.stop();
          process.exit(1);
        }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // The constructor has no arguments. The initial private ballot stays
      // local; the deployed ledger starts with zero turnout and no commitments.
      deployed = await deployContract(providers, {
        compiledContract: compiledContract as any,
        args: [],
        privateStateId: PRIVATE_STATE_ID,
        initialPrivateState: { choice: false, nonce: randomBytes(32) },
      });
      break;
    } catch (err: any) {
  ...
  type PublicDeployment = { address: string; deployer: string; deployedAt: string };
  const previous = fs.existsSync(deploymentPath)
    ? JSON.parse(fs.readFileSync(deploymentPath, 'utf8')) as Record<string, unknown>
    : {};
  // Preserve the first Preview deployment, which used a single-record format.
  const deployments: Record<string, PublicDeployment> =
    typeof previous.network === 'string'
      ? { [previous.network]: {
          address: previous.address as string,
          deployer: previous.deployer as string,
          deployedAt: previous.deployedAt as string,
        } }
      : previous as Record<string, PublicDeployment>;
  deployments[network] = {
  ...
  console.log('  Saved to deployment.json in the project root\n');
