...
export async function createWallet(opts: CreateWalletOptions): Promise<WalletContext> {
  ...
  const walletConfig = {
    ...
    // Preprod has a long DUST history. Larger batches avoid a delay after
    // every ten events while the wallet catches up from a fresh seed.
    batchUpdates: { size: 500, timeout: 10, spacing: 0 },
